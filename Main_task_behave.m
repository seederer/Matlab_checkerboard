...
%% get subject info
prompt = {'Subject', 'Session' ,'Block', 'Sex', 'Hand', 'Age'};

%fill in some stock answers to the gui input boxes
defAns = {'10', '4', '1' , 'm',  'r', '31'};
...
cName=[p.root, '/Subject_Data/codeMainTask_behav' '_sbj', num2str(p.subNum), '_sess', num2str(p.sessType), '.mat'];
...
p.tasktype = ctask(p.runNum);


fName=[p.root, '/Subject_Data/MainTask_behav' '_sbj', num2str(p.subNum), '_sess', num2str(p.sessType), '_block', num2str(p.runNum), '.mat'];
...
load correctGammarmB_01122017
p.LUT = correctGamma*255;

% window characteristics:
screen=max(Screen('Screens'));
[win, scr_rect] = Screen('OpenWindow', 1);
...
Rring = 15;
Rstim = 60; % radius of stimulus (px)
...
respCode =KbName('m'); %dectection button
abortKey = KbName('q');

ct = [.94 .93 0.92 .89 0.85 .80 1];
...
if ismember(p.tasktype, 1:6)
    p.stimloc = [repmat(tgloc, [1,6]), ones(1,72)*tgloc(p.tasktype) repmat(tgloc, [1, 9])];
    p.tgntg = [ones(1, 6) ones(1, 6)*2 ones(1, 6)*3 ones(1, 6)*4 ones(1, 6)*5 ones(1, 6)*6 ...
        repmat(1:6, [1, 12]) zeros(1, 54)];
elseif p.tasktype >= 7
    p.stimloc = [repmat(tgloc, [1,18]) repmat(tgloc, [1, 9])];
    p.tgntg = [ones(1, 18) ones(1, 18)*2 ones(1, 18)*3 ones(1, 18)*4 ones(1, 18)*5 ones(1, 18)*6 zeros(1, 54)];
end
...
for t = 1:p.tnumb
    ...
    while p.stimdur(t) <= stimDur
        ...
        if p.tasktype <=7
            ...
            Screen('DrawTexture', win, fixtext{7}, ...
                [], [xcenter-Rfix ycenter(1)-Rfix xcenter+Rfix ycenter+Rfix],0); %fixation
        else
            Screen('DrawTexture', win, checktext{7}, ...
                [],[LocX(stimlocx)-Rstim LocY(stimlocx)-Rstim LocX(stimlocx)+Rstim LocY(stimlocx)+Rstim], 0);
            Screen('FillOval',win,background,[xcenter-Rring ycenter(1)-Rring xcenter+Rring ycenter+Rring],[]); %fixation
...
DrawFormattedText(win,['hit =' num2str(hit_circle) ' / false alarm =' num2str(fa_circle) ], 'center', ycenter+20 ,  black);
Screen('DrawTexture', win, fixtext{1, 1}, ...
    [], [xcenter-Rfix ycenter(1)-Rfix xcenter+Rfix ycenter+Rfix],0); %fixation
Screen(win, 'Flip');
KbWait;
